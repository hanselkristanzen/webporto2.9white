...
export function TargetCursor({
  ...
  // PERFORMANCE: tickerFn (below) runs on every GSAP ticker tick while a
  // target is actively hovered, and used to call `Array.from(cornersRef
  // .current)` on every single one of those ticks — a fresh array
  // allocation every tick purely to re-derive something that never
  // changes after setup (there are always exactly 4 corner elements,
  // queried once). Cached here alongside cornersRef instead.
  const cornersArrayRef = useRef<HTMLDivElement[]>([]);
  ...
  useEffect(() => {
    ...
    cornersArrayRef.current = Array.from(cornersRef.current);
    ...
    const tickerFn = () => {
      ...
      const corners = cornersArrayRef.current;
